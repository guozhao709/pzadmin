    ...
    >
        ...
        >
            ...
            <el-form-item label="是否生效" prop="active">
                <el-radio-group v-model="form.active" prop="active">
        ...
        if (valid) {
            companion(form).then(({ data }) => {
                if (data.code === 10000) {
                    ...
                    console.log(data.message, "companionError");
                }
            });
        } else {
...
const handleSelectionChange = (val) => {
    selectTabData.value = val;
}

// 列表删除
const confirmEvent = ()=>{
    if(!selectTabData.value.length){
        return alert('请选择至少一项数据')
    }
    deleteCompanion({id:selectTabData.value}).then(({ data }) => {
        if (data.code === 10000) {
            getListData();
        }
    })
